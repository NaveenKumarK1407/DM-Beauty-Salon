...
export async function getHomepagePromotion() {
  if (isConfigured()) {
    ...
    const doc = await db.collection('settings').doc('homepagePromotion').get();
    return doc.exists ? doc.data() : null;
  }
  return (await readLocal('homepagePromotion'))[0] || null;
}

export async function saveHomepagePromotion(promotion) {
  const data = { ...promotion, updatedAt: new Date().toISOString() };
  if (isConfigured()) {
    const db = await getDb();
    await db.collection('settings').doc('homepagePromotion').set(data);
  } else {
    await writeLocal('homepagePromotion', [data]);
  }
  return data;
}
